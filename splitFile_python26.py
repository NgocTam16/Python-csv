...
Run on terminal: python splitFile_python26.py server_services.csv 130000
***/
'''
import csv
import sys
...
arg_fullname = sys.argv[1]
arg_row_limit = sys.argv[2]
name = re.split('\.', arg_fullname)[0]

def split_big_file(filename = arg_fullname, row_limit = arg_row_limit):
	...
	for index, row in enumerate(readSource):
		if index % int(row_limit) == 0:
			...
			outfilename = '%s-{0}.csv'.format(outfileNumber) % name
			# open file to write
